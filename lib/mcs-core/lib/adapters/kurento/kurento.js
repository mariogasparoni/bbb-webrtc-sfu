...
module.exports = class Kurento extends EventEmitter {
  ...
  /**
   * Set the current media as the video floor of the mixer which belongs to the
   * given room
   * @param  {String}  mixerId The id of the mixer
   * @param  {Number}  layoutId The layout id
   * @return {Promise}       A Promise for this process
   */
  async setLayoutTypeByMixerId (mixerId, layoutId) {
    try {
      if (!mixerId) {
        Logger.debug(LOG_PREFIX, 'Could not set layout type, invalid mixerId');
        return;
      }
      ...
      let mixerElement = this._mediaElements[mixerId];

      if (!mixerElement) {
        Logger.error(LOG_PREFIX, 'Could not set layout type, invalid ' +
          'mixerElement: ' + mixerId);
      }

      Logger.info(LOG_PREFIX, "Setting layout type to", layoutId);
  ...
  async setLayoutType (media, layoutId) {
    try {
      ...
      return this.setLayoutTypeByMixerId(media.mixerId);
    } catch (error) {
      throw (this._handleError(error));
    }
  }
