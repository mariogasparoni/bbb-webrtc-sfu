...
class MCUSession extends MediaSession {
  ...
  async setLayout (layoutId) {
    try {
      ...
      // We currently set the layout for the default video mixer , only
      const mcuMedia = this.medias ? this.medias[0] : null;

      return videoAdapter.setLayoutTypeByMixerId(mcuMedia.adapterElementId,
        layoutId);
