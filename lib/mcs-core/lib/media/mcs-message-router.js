...
const MR = class MCSRouter {
  ...
  setLayout (args) {
    const { roomId, layoutId } = args
    try {
      return this._mediaController.setLayout(roomId, layoutId);
    }
    catch (error) {
      throw (this._handleError(error, 'setLayout', { roomId, layoutId }))
    }
  }
  ...
  setupClient(client) {
    ...
    client.on('setLayout', async (args) => {
      let roomId, layoutId, transactionId;
      try {
        ({roomId, layoutId, transactionId } = args);
        const newLayoutId = await this.setLayout(args)
        client.layoutChanged(roomId, newLayoutId, { transactionId })
      }
      catch (e) {
        this._notifyMethodError(client, e, transactionId)
      }
