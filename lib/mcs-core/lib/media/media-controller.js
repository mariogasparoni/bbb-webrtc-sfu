...
module.exports = class MediaController {
  ...
  async publishAndSubscribe (roomId, userId, sourceId, type, params = {}) {
    ...
    try {
      source = room.getDefaultMcuMediaSession();
      if (!source) {
      ...
      // Not a fatal operation. Log it and proceed. Rollback the source var
      // to avoid connecting nonsense in an unexpected state
      const normalizedError = this._handleError(error);
      Logger.warn(LOG_PREFIX, `PublishAndSubscribe for ${userId} failed to fetch subscription source`, { roomId, userId, error: normalizedError });
      source = null;
    }
    ...
    // If a sourceId was specified or the user is an MCU user, try to connect
    // the source to the newly created session
    if (source) {
      try {
        if (user.type === C.USERS.MCU) {
          await source.connect(session);
          await this._connectContentMediaSession(room, session);
        ...
        const normalizedError = this._handleError(error);
        Logger.error(LOG_PREFIX, `Error on PublishAndSubscribe subscription to ${sourceId} from ${session.id}`, { error: normalizedError });
      }
    }
  ...
  async publish (userId, roomId, type, params = {}) {
    ...
    this.addMediaSession(session);
    room.addMediaSession(session);
    const mcuSession = room.getDefaultMcuMediaSession();
    if (mcuSession) {
      try {
        await session.connect(mcuSession);
      } catch (error) {
        // Not a fatal operation. Log the error and go on.
        Logger.error(LOG_PREFIX, `Failed to connect new published media ${session.id} to MCU session due to ${error.message}`,
          { roomId, userId, mediaId: session.id, error });
      }
    }
  ...
  async setLayout (roomId, layoutId) {
    try {
      const room = this.getRoom(roomId);

      const mcuMediaSession = room.getDefaultMcuMediaSession();
      return mcuMediaSession.setLayout(layoutId);
    } catch (error) {
      Logger.error(LOG_PREFIX, `setLayout for room ${roomId} to ${layoutId} failed due to ${error.message}`,
        { roomId, layoutId, error });
      throw (this._handleError(error));
    }
  }
  ...
  async setContentFloor (roomId, mediaId) {
    try {
      ...
      // FIXME connecting existing content media sessions here seems rather
      // arbitrary and an implementation side effect.
      // Consider reviewing this - prlanzarin 2020-01-21
  ...
  /**
   * Connect current Content Media Session of the room in the given media
   * @param  {Room}  room     Room object , where the media sessions are
   *                          going to be connected to
   * @param  {MediaSession}  mediaSession     MediaSession object,
   *                         which will receive video from the current
   *                         media
   * @return {Promise}       A Promise for this process
   */
  async _connectContentMediaSession (room, mediaSession) {
    try {
      if (!room) {
        throw C.ERROR.ROOM_NOT_FOUND;
      }

      let contentMediaSession = room.getContentFloorMedia();

      if (!mediaSession || !contentMediaSession) {
        Logger.warn(LOG_PREFIX, ' Could not connect connect media session',
          contentMediaSession ? contentMediaSession.id : 'null', ' ->',
          mediaSession ? mediaSession.id : 'null', ': invalid media.');
        return;
      }

      let contentMedia = contentMediaSession.getContentMedia();
      if (contentMedia) {
        await contentMedia._connect(mediaSession, C.CONNECTION_TYPE.CONTENT);
      }
    } catch (error) {
      throw this._handleError(error);
    }
